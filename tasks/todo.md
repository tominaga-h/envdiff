...
- [ ] **Task 1** — Cargo.toml 依存追加 + モジュール骨格（§7） · S · deps: なし
- [ ] **Task 2** — パーサ: 基本形・export・コメント・行番号・パースエラー（§4.1-4.3, §4.8） · M · deps: 1
- [ ] **Task 5** — 比較ロジック: 4 分類 + 出力順序（§5.1, §5.2, §5.3） · S · deps: 2
- [ ] **Task 6** — CLI 骨格: 引数・ファイル読み込み・終了コード（§2, §3） · M · deps: 2, 5
- [ ] **Task 7** — テーブル出力（§6.1） · M · deps: 6

### ⛳ Checkpoint A
- [ ] `cargo build` が警告なしで通る / `cargo test` が全て通る
- [ ] 手動確認: `.env` 2 つを比較してテーブルが出て exit 1
- [ ] 手動確認: 出力の並びが A の記載順と一致する（§5.3）
- [ ] **人間レビュー: 実際の出力を見て §5.3 の順序が読みやすいか**
